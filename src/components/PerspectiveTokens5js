    ...
    const Token = ({name}) => {
      ...
      useEffect(() => {
        ...
        Object.keys(data).length == 0 && (async() => {
          ...
          let tt = await t.json();
          getData(tt.data[0]);
          console.log("DATA IS UPDATES ", data)
        })()
      },[data])

      return (<>
          <div style={isInteresting ? {background: "#303030"} : {background: "#202020"} }>
          <div>
            name: {name}
          </div>
          <div>
      ...
      <div>
        ...
        <div>
          {Object.keys(STATE.perspectiveTokens).map(token => (
            ...
            <button onClick={() => dispatcher("SAVE_INTERESTING_TOKEN",token)}>add</button>
            <div>
                INTEREST: {showStatus(token)}
            </div>
            {  showStatus(token) &&
                <>
                  <button onClick={() => dispatcher("BUY_INTERESTING_TOKEN",token)}>buy</button>
                  <button onClick={() => dispatcher("REMOVE_INTERESTING_TOKEN",token)}>remove</button>
                </>
            }
            </>
          ))}
